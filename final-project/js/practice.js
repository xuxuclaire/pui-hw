...
//check if currentQuestionCount is already in storage
if (localStorage.getItem('currentQuestionCount') != null){
    currentQuestionCount = Number(localStorage.getItem("currentQuestionCount"));
}
...
//check if practiceList is already saved in localStorage
if (localStorage.getItem('practiceList') != null){
    getPracticeList(practiceList);
}else{
    console.log(tempList["question"]);
      
    for (let i = 0; i < practiceAmount; i += 1){
        let length = tempList["question"].length - 1;
        let tempNum = Math.abs(((Math.random() * length)).toFixed());
        console.log(tempNum);
...
function goToNextQuestion(){
    ...
    checkLastQuestion();
    addToSaved.style.backgroundColor = '#9A9999';
}

//remove next question if this is the last question
...
function cameraButtonPressed(){
    ...
    if(cameraButtonIcon.alt == 'camera-on'){
        ...
        cameraButtonIcon.src = "../assets/camera icon gold.png";
        cameraButton.style.borderColor = "darkorange";
        ...
        cameraButtonIcon.src = "../assets/camera icon.png"
        cameraButton.style.borderColor = "initial";
    }
}
...
function turnOnVideo(){
    ...
    questionDisplay.style.height = "auto";
...
function turnOffVideo(){
    ...
    questionDisplay.style.marginBottom = "40px";
    questionDisplay.style.height = "330px";

    questionInfo.style.marginBottom = "30px";
  
    practiceQuestion.style.fontSize = "48px";
}
...
function submitQuestion(){
    addToSaved.style.backgroundColor = "DarkOrange";
...
function goToSavedList(){
    //save current progress: practiceList
    savePracticeList(practiceList);
    localStorage.setItem("currentQuestionCount", currentQuestionCount);
...
function savePracticeList(list){
    const practiceListArray = Array.from(list);
    console.log(practiceListArray);
    const practiceListArrayString = JSON.stringify(practiceListArray);
    localStorage.setItem('practiceList', practiceListArrayString);
}

function getPracticeList(list){
    const practiceListArrayString = localStorage.getItem('practiceList');
    const practiceListArray = JSON.parse(practiceListArrayString);
    for (let itemData of practiceListArray){
        list.push(itemData);
    }
}
